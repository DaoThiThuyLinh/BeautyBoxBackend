...
public interface OrderRepository extends JpaRepository<OrderProduct, String> {
    ...
    @Query(value = "SELECT DISTINCT op " +
            ...
            "AND op.createdAt >= :fromDate " +
            "AND op.createdAt <= :toDate ")
    Page<OrderProduct> getOrders(String s, String userId, int status, Pageable pageable, LocalDateTime fromDate, LocalDateTime toDate);
